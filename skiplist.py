...
class SkipList():
    ...
    def insert(self, key, value, toplevel):
        newNode = Node(key=key, value=value, toplevel=toplevel, pointers=[None] * (toplevel + 1))
        curr = self.headnode

        
        update = [None] * (self.maxlevel + 1)
        for i in range(self.maxlevel, -1, -1):
            while curr.pointers[i] is not None and curr.pointers[i].key < key:
                curr = curr.pointers[i]
            update[i] = curr

        
        for i in range(toplevel, -1, -1):
    ...
    def delete(self,key):
        ...
        for i in range(self.maxlevel, -1, -1):
            while curr.pointers[i] is not None and curr.pointers[i].key < key:
                curr = curr.pointers[i]
            update[i] = curr
        nextNode = curr.pointers[0]
        if nextNode is not None and nextNode.key == key:
            for i in range(len(nextNode.pointers)):
                update[i].pointers[i] = nextNode.pointers[i]
    ...
    def search(self,key) -> str:
        ...
        for i in range(self.maxlevel, -1, -1):
            while curr.pointers[i] is not None and curr.pointers[i].key < key:
                curr = curr.pointers[i]
                A.append(curr.key)
            if curr.pointers[i] is not None and curr.pointers[i].key == key:
                A.append(curr.pointers[i].key)
                A.append(curr.pointers[i].value)
