...
public class PublicTests {
	...
	public void testListRemove() {
		...
		assertEquals("[ Cereal, Coffee, Croissant, Donut, Egg, Hashbrowns, Melon, Milk, Orange Juice, Pancakes, Pie, Toast ]", list.toString());
	...
	@Test
	public void testCheckAvalability() {
		SortedListOfImmutables list = new SortedListOfImmutables();
		for (int i = Food.FOOD_OBJECTS.length - 1; i >= 0; i--) {
			list.add(Food.FOOD_OBJECTS[i]);
		}
		...
		assertEquals(list.checkAvailability(BACON), false);
		...
		list2.add(BACON);
		list2.remove(EGG);
		assertTrue(list2.checkAvailability(BACON)==true);
		assertTrue(list2.checkAvailability(EGG)== false);
