...
public class Model {
	...
	public boolean isSpaceAvailable(int r, int c) {
		...
		for(Plant newPlant: plants) {
			if(newPlant.getCol()==c && newPlant.getRow()==r) {
				plantExists=true;
				break;
			}
		...
		for(Fish newFish: fish) {
			if(newFish.getCol()==c && newFish.getRow()==r) {
				fishExists=true;
				break;
			}
		}
		if(plantExists||fishExists||landscape[r][c]==ROCK ) {
			return false;
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		...
		int currentCol=f.getCol();
		if (landscape[currentRow][currentCol-1]==ROCK && landscape[currentRow][currentCol+1]==ROCK
				&& landscape[currentRow+1][currentCol]==ROCK && landscape[currentRow-1][currentCol]==ROCK) {
			return true;
		}else {
			return false;
		}
	...
	public void turnFish() {
		...
		for(Fish newFish: fish) {
			if(newFish.isAlive()==true) {
				if(fishIsSurroundedByRocks(newFish)==true) {
					continue;
				}
				
				int currentRow=newFish.getRow();
				int currentCol=newFish.getCol();
				if(newFish.getDirection()==Fish.DOWN) {
					currentRow++;
				}else if(newFish.getDirection()==Fish.LEFT) {
					currentCol--;
					
				}else if(newFish.getDirection()==Fish.UP) {
					currentRow--;
				}else if(newFish.getDirection()==Fish.RIGHT) {
					currentCol++;
				}else {
					throw new IllegalFishDirectionException(newFish.getDirection());
				}
				while(landscape[currentRow][currentCol]==ROCK &&
						!fishIsSurroundedByRocks(newFish)) {
					newFish.setRandomDirection();
					currentRow=newFish.getRow();
					currentCol=newFish.getCol();
					if(newFish.getDirection()==Fish.DOWN) {
						currentRow++;
					}else if(newFish.getDirection()==Fish.LEFT) {
						currentCol--;
	...
	public void moveFish() {
		for(Fish newFish: fish) {
			if(newFish.isAlive()) {
				if (fishIsSurroundedByRocks(newFish)) {
					continue;
			}
				...
				for(Plant newPlant: plants) {
						...
						if(newPlant.getCol()==newFish.getCol() && newPlant.getRow()==newFish.getRow() &&
								newPlant.isAlive()) {
							fishEatPlant(newFish, newPlant);
						}
					}
					for(int otherFishPosition=0; otherFishPosition<fish.size(); otherFishPosition++) {
						if(newFish.getCol()==fish.get(otherFishPosition).getCol() && 
								newFish.getRow()==fish.get(otherFishPosition).getRow() && 
								fish.indexOf(newFish)!=otherFishPosition) {
							newFish.fight(fish.get(otherFishPosition));
	...
	public void addPlant(Plant p) {
		...
		for(int otherPlantPos=0; otherPlantPos<plants.size(); otherPlantPos++) {
			if(plants.get(otherPlantPos).getRow()==p.getRow()&& plants.get(otherPlantPos).getCol()==p.getCol()) {
	...
	public void addFish(Fish f){
		...
		for(int otherFishPos=0; otherFishPos<fish.size(); otherFishPos++) {
			if(fish.get(otherFishPos).getRow()==f.getRow()&& fish.get(otherFishPos).getCol()==f.getCol()) {
